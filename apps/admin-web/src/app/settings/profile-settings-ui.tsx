...
function statusTone(status?: string) {
  ...
  if (["ACTIVE", "READY", "CONNECTED", "CURRENT", "PRO", "DONE"].includes(s)) return "good";
  if (["FAILED", "ERROR", "SUSPENDED", "CANCELLED"].includes(s)) return "bad";
  return "warn";
}
...
function setBool(settings: StoreSettings, key: keyof StoreSettings, value: boolean) {
  return { ...settings, [key]: value ? "true" : "false" };
}
...
export default function ProfileSettingsUI() {
  ...
  const setupScore = useMemo(() => {
    const checks = [
      Boolean(org?.name && org.phone),
      Boolean(payment.promptPayId || payment.accountNo),
      Boolean(settings.slug),
      Boolean(settings.logoDataUrl),
      Boolean(settings.contactLine || settings.supportPhone),
      integrations.length > 0,
    ];
    return Math.round((checks.filter(Boolean).length / checks.length) * 100);
  }, [org, payment, settings, integrations]);

  function patchOrg(key: keyof StoreProfile["organization"], value: string) {
  ...
  async function uploadLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setSettings((current) => ({ ...current, logoDataUrl: saved.logoDataUrl }));
    ...
    try {
      if (org) {
        await api("/store/profile", {
          method: "PATCH",
          body: JSON.stringify({ name: org.name, ownerName: org.ownerName, phone: org.phone, taxId: org.taxId, address: org.address, billingEmail: org.billingEmail }),
        });
      }
      ...
      setNotice("บันทึกโปรไฟล์และระบบตั้งค่าแล้ว");
      await load();
    } catch (e) {
  ...
  function toggleDay(day: string) {
    ...
    setSettings({ ...settings, openDays: next.join(",") });
  }

  function channelChip(key: "notifyLine" | "notifySms" | "notifyEmail", label: string) {
    const active = on(settings[key]);
    return <button type="button" className={`channel ${active ? "active" : ""}`} onClick={() => setSettings(setBool(settings, key, !active))}>{label}<span>{active ? "✓" : "+"}</span></button>;
  }

  return (
    <main className={`profile-console ${theme} ${collapsed ? "collapsed" : "open"}`}>
      <ProfileCss />
      <aside className="profile-side">
        <button className="side-brand" type="button" onClick={() => setCollapsed(!collapsed)}><span>K</span><b>Store</b></button>
        <nav>
          {nav.map((item) => <button key={item.id} type="button" className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><i>{item.icon}</i><b>{item.label}</b></button>)}
        </nav>
        <div className="side-footer">
          <button type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")}><i>{theme === "dark" ? "☾" : "☀"}</i><b>{theme === "dark" ? "Dark" : "Light"}</b></button>
          <a href="/"><i>⌂</i><b>กลับ Store</b></a>
        </div>
      </aside>

      <section className="profile-main">
        <header className="topbar">
          <div className="top-title"><span className="k-mini">K</span><h1>Store Profile</h1><span className="api-dot">API ready</span></div>
          <div className="top-actions"><button type="button" onClick={load}>{busy === "load" ? "Loading" : "Refresh"}</button><a href="/">ออก</a></div>
        </header>

        <section className="hero-card">
          <div className="avatar-wrap">
            <button type="button" className="avatar" onClick={() => fileRef.current?.click()}>{logo ? <img src={logo} alt="Store logo" /> : <span>K</span>}<i>📷</i></button>
            <input ref={fileRef} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadLogo} />
          </div>
          <div className="hero-info"><h2>{org?.name || "Koga Mobile Store"}</h2><p>{org?.ownerName || "Owner"}</p><div className="badges"><span className="badge pro">{org?.plan || "PRO"}</span><span className={`badge ${statusTone(org?.status)}`}>{org?.status || "ACTIVE"}</span></div></div>
          <div className="hero-stats"><Stat label="ลูกค้า" value={profile?.counts.customers ?? 0} /><Stat label="อุปกรณ์" value={profile?.counts.devices ?? 0} /><Stat label="สัญญา" value={profile?.counts.contracts ?? 0} /><div className="stat progress"><b>{setupScore}%</b><span>ตั้งค่าสำเร็จ</span><em style={{ background: `conic-gradient(#7c3aed ${setupScore * 3.6}deg, rgba(148,163,184,.18) 0)` }} /></div></div>
        </section>

        <section className="theme-switch"><button className={theme === "dark" ? "active" : ""} onClick={() => setTheme("dark")}>☾ Dark</button><button className={theme === "light" ? "active" : ""} onClick={() => setTheme("light")}>☀ Light</button></section>

        {notice && <div className="alert good">{notice}</div>}
        {error && <div className="alert bad">{error}</div>}

        <form className="profile-grid" onSubmit={saveAll}>
          <section className={`panel span-2 ${section !== "store" ? "ghosted" : ""}`}>
            <PanelTitle icon="▣" title="ข้อมูลร้าน" />
            <div className="fields two"><Field label="ชื่อร้าน" value={org?.name} onChange={(v) => patchOrg("name", v)} /><Field label="เจ้าของร้าน" value={org?.ownerName} onChange={(v) => patchOrg("ownerName", v)} /><Field label="เบอร์โทร" value={org?.phone} onChange={(v) => patchOrg("phone", v)} /><Field label="อีเมลรับบิล" value={org?.billingEmail || org?.email} onChange={(v) => patchOrg("billingEmail", v)} /><Field label="เลขภาษี" value={org?.taxId} onChange={(v) => patchOrg("taxId", v)} /><Field label="เว็บไซต์" value={settings.website} onChange={(v) => setSettings({ ...settings, website: v })} /></div>
            <Field textarea label="ที่อยู่" value={org?.address} onChange={(v) => patchOrg("address", v)} />
            <div className="logo-row"><div className="logo-preview">{logo ? <img src={logo} alt="logo" /> : <span>K</span>}</div><div><b>โลโก้ร้าน</b><p>PNG, JPG, WEBP ไม่เกิน 1MB</p><button type="button" onClick={() => fileRef.current?.click()}>{busy === "logo" ? "Uploading..." : "เปลี่ยนโลโก้"}</button></div></div>
            <Field label="เวลาทำการ" value={settings.businessHours} onChange={(v) => setSettings({ ...settings, businessHours: v })} />
            <div className="days">{days.map((day) => <button type="button" key={day} className={openDays.includes(day) ? "active" : ""} onClick={() => toggleDay(day)}>{day}</button>)}</div>
          </section>

          <section className={`panel ${section !== "payment" ? "ghosted" : ""}`}><PanelTitle icon="฿" title="รับเงิน" /><Field label="ชื่อที่แสดง" value={payment.displayName} onChange={(v) => setPayment({ ...payment, displayName: v })} /><Field label="PromptPay" value={payment.promptPayId} onChange={(v) => setPayment({ ...payment, promptPayId: v })} /><Field label="ธนาคาร" value={payment.bankName} onChange={(v) => setPayment({ ...payment, bankName: v })} /><Field label="เลขบัญชี" value={payment.accountNo} onChange={(v) => setPayment({ ...payment, accountNo: v })} /><Field label="ชื่อบัญชี" value={payment.accountName} onChange={(v) => setPayment({ ...payment, accountName: v })} /><Toggle label="เปิดใช้งาน QR Payment" checked={on(settings.qrPaymentEnabled)} onChange={(v) => setSettings(setBool(settings, "qrPaymentEnabled", v))} /><Field textarea label="ข้อความท้ายบิล" value={settings.invoiceFooter || payment.instructions} onChange={(v) => { setSettings({ ...settings, invoiceFooter: v }); setPayment({ ...payment, instructions: v }); }} /></section>
          ...
          <section className={`panel ${section !== "notify" ? "ghosted" : ""}`}><PanelTitle icon="◌" title="แจ้งเตือน" /><div className="channels">{channelChip("notifyLine", "LINE")}{channelChip("notifySms", "SMS")}{channelChip("notifyEmail", "Email")}</div><div className="template-list">{templates.slice(0, 4).map((t) => <div key={`${t.key}-${t.channel}`}><b>{t.title}</b><span>{t.channel}</span></div>)}</div></section>
          ...
          <section className={`panel wide ${section !== "security" ? "ghosted" : ""}`}><PanelTitle icon="◇" title="ความปลอดภัย" /><div className="security-grid"><Toggle label="ยืนยันตัวตน 2 ขั้นตอน (2FA)" checked={on(settings.twoFactorEnabled)} onChange={(v) => setSettings(setBool(settings, "twoFactorEnabled", v))} /><Toggle label="แจ้งเตือนการเข้าสู่ระบบ" checked={on(settings.loginAlerts)} onChange={(v) => setSettings(setBool(settings, "loginAlerts", v))} /><Toggle label="ควบคุมเซสชัน" checked={on(settings.sessionControl)} onChange={(v) => setSettings(setBool(settings, "sessionControl", v))} /><Field label="Roles Preset" value={settings.rolePreset} onChange={(v) => setSettings({ ...settings, rolePreset: v })} /></div></section>
